...
originalProductList1=[{
 ...
 activeFilter= 'all';
  ...
  ngOnInit(): void {
    for(let product of this.originalProductList){
      product['step1']=false;
      product['step2']=false;
      product['sizestring']='';
      if(this.filterOptions.indexOf(product['tag'])==-1){
        this.filterOptions.push(product['tag'])
      }
      for(let variable of product['options']){
        variable['displayName']=this.processOptionName(variable['value']);
        if(product['sizestring']==''){
          product['sizestring']=variable['displayName']
        }else{
          product['sizestring']=product['sizestring']+', '+variable['displayName']
        }
      }
      console.log(product)
    }
    this.productToShow = this.originalProductList;
  }
  ...
  onFilterClick(tagName){
      this.activeFilter = tagName;
      if(tagName == 'all'){
          this.productToShow = this.originalProductList;
          return;
      }
    this.productToShow = this.originalProductList.filter((element,index)=>{
  ...
  onMouseOver(pId,product){
    console.log(pId);
    this.currentProductHoverIndex = pId;
    if(!product['stpe2']){
      product['step1']=true
    }
  }

  productSelected(product,variableIndex){
    
    
    this.selectedProductVariable = product['options'][variableIndex];
    product['step2']=true
    product['step1']=false
    console.log(variableIndex,this.currentProductHoverIndex,product['step1'],product['step2'])
  }

  onMouseOut(pId ,product){
    // if(this.currentProductHoverIndex==pId){
    // this.currentProductHoverIndex=-1
    // this.selectedProductVariable = null;
    // this.showAddToCartButton = false;
    // product['step2']=false;
    // product['step1']=false;
    // console.log('out',this.currentProductHoverIndex,product['step1'],product['step2'])
    // }
  }

  addToCart(product){
    console.log(product,this.selectedProductVariable)
    product['step2']=false;
